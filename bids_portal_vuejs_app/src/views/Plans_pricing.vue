    ...
    <section class="pt-4 pt-md-5 pb-0" id="plans_pricing">
        <div class="container fs-18">
            ...
            <div class="row justify-content-center my-5">
                <div class="col-sm-5" v-for="plan,key in plans" :key="key">
                    <div class="card h-100" :class="plan.discount!=0 ? 'bg-light' : ''">
                        <div class="card-header  pb-0" :class="plan.discount!=0 ? 'bg-light' : ''">
                            <h5 class="mb-0 text-success ">{{ plan.plan }} Subscription</h5>
                        </div>
                        <div class="card-body p-4">
                            <p>{{ plan.month_desc }}</p>
                        </div>
                        <div class="card-footer d-flex justify-content-between align-items-center p-4 pt-0" :class="plan.discount!=0 ? 'bg-light' : ''">
                            <div>
                                <div class="d-flex align-items-center">
                                    <h5 class="fw-bold mb-3 me-1">${{plan.price}}/{{ plan.plan }}</h5>
                                </div>
                                <a href="#" class="btn btn-primary mb-2" :disabled="$store.getters.user?.subscription_id !==0" @click.prevent="getplan(plan.id)">SUBSCRIBE NOW</a>
                            </div>
                            <div v-if="plan.discount!=0" class="position-absolute top-100 start-100 z-index-9" style="margin-left: -8rem; margin-top: -6rem;">
                    ...
                    <p class="text-end text-dark fnt-wt-500" v-if="plan.discount!=0">
                        Save {{ plan.discount_per }}% by paying {{ plan.plan }}
                    </p>
                </div>
            </div>
