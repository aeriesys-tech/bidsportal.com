<template>
    <div class="card card-body rounded-0 p-2">
        <div class="ml2 d-flex btn-more collapsed d-flex justify-content-between align-items-center" data-bs-toggle="collapse" href="#noticetype" role="button" aria-expanded="false" aria-controls="status">
            <h6 style="font-size: 15px; font-weight: normal;" class="mb-2" :class="{bluetextclass: meta.international_notices?.length !== 0}">Notice Type</h6>
        ...
        <div class="col-12 ml2 multi-collapse collapse show" v-for="notice in international_notices" :key="notice.international_notice_id">
            <div class="d-flex justify-content-between align-items-center">
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" v-model="meta.international_notices" :value="notice.international_notice_id" />
    ...
    <div class="card card-body rounded-0 p-2">
        <div class="ml2 d-flex btn-more collapsed d-flex justify-content-between align-items-center" data-bs-toggle="collapse" href="#federalagency" role="button" aria-expanded="false" aria-controls="notice">
            <h6 style="font-size: 15px; font-weight: normal;" class="mb-2" :class="{'bluetextclass': meta.international_agencies?.length !==0}">State Agency</h6>
        ...
        <div class="col-12 ml2 multi-collapse collapse show">
            <div class="d-flex justify-content-between align-items-center">
                <div class="mb-1">
                    <a class="btn btn-link p-0 mb-0 me-2">({{ meta.international_agencies.length }} of {{ international_agencies.length }})</a>
                    <span v-if="meta.international_agencies.length !== sorted_international_agencies.length">
                    ...
                    <span v-if="meta.international_agencies.length">
                        <a href="" class="form-check-label text-primary me-2" @click.prevent="deselectAllStateAgencies()" v-if="meta.international_agencies.length !== 0">| Reset</a>
                    </span>
                </div>
            </div>
            <div class="d-flex justify-content-between align-items-center">
                <form class="position-relative w-100 me-4 mb-2">
                    <input class="form-control form-control-sm bg-light pe-5" type="search" @keyup="searchStateAgencies()" v-model="international_agency_keword" placeholder="Search Agency" aria-label="Search" />
                    <button class="bg-transparent px-2 py-0 border-0 position-absolute top-50 end-0 translate-middle-y" type="submit"><i class="fas fa-search fs-6 text-primary"></i></button>
                </form>
            </div>

            <div class="scroll-sidebar">
                <div class="d-flex justify-content-between align-items-center" v-for="international_agency in sorted_international_agencies" :key="international_agency.international_agency_id">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" :value="international_agency.international_agency_id" v-model="meta.international_agencies" />
                        <label class="form-check-label"> {{ international_agency.international_agency_name }}</label>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    data(){
        return{
            ...
            state_keyword: null,
            errors:[],
            international_agency_keword:null
        }
    },
    ...
    watch:{
        ...
        'international_agencies' : function(){
    ...
    methods:{
        ...
        getInternationalNotices() {
            let vm = this
            vm.international_notices = vm.$store.getters.international_notices
            if(!vm.international_notices.length){
                vm.$store
        ...
        getInternationalAgencies() {
            let vm = this;
            vm.international_agencies = vm.$store.getters.international_agencies
            if(!vm.international_agencies.length){
                let uri = "getInternationalAgencies";
                vm.$store
                    .dispatch("post", { uri: uri, data: {country_id:1} })
                    .then(function (response) {
                        vm.international_agencies = response.data.data
                        vm.$store.dispatch('setInternationalAgencies', vm.international_agencies)
                    })
                    .catch(function (error) {
                        console.log(error)
                    });
