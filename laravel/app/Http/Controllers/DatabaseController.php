...
class DatabaseController extends Controller
{
    ...
    {
        DB::connection('bidsdb')->table('tbl_tender')->where('tdr_region', 2)
    ...
    {
        DB::connection('bidsdb')->table('tbl_tender')->where('tdr_region', 1)
        ->orderBy('tdr_no')->chunk(100, function ($tenders) {
            $jobData = $tenders->map(function ($tbl) {
                return [
                    'tdr_no' => $tbl->tdr_no,
    ...
    {
        $users = DB::connection('bidsdb')->table('tbl_users')->get();
            ...
            if(!$user_exists){
                ...
                if($user->socioeconomic_status){
                    ...
                    foreach ($set_asides as $set_aside_id) {
                        $set_aside_old = DB::connection('bidsdb')->table('tbl_set_aside_status')->where('status_id', $set_aside_id)->first();
                        if($set_aside_old){
                ...
                $user_subscription_old = DB::connection('bidsdb')->table('user_subscriptions')->where('user_id', $user->id)->first();
