...
class PrivateTenderImport implements ToCollection, WithValidation, WithStartRow
{
    ...
    {
        $this->existing_tender_nos = PrivateTender::pluck('tender_no')->toArray();
        ...
        foreach ($rows as $key => $row) {
            ...
            try {
                ...
                if (in_array($tender_no, $this->existing_tender_nos)) {
                    DuplicatePrivateTender::updateOrCreate([
                        'tender_no' => $tender_no,
                ...
                $tdr_fees = 0;
                try{
                    $private_tender = PrivateTender::updateOrCreate(
                        [   'tender_no' => $tender_no ],
                        [
                            'title' => $title,
                            ...
                            'state_id ' => null,
                            'tender_type_id' => null,
                            'private_notice_id' => null,
                            'category_id' => null,
                            'private_agency_id' => null,
                            'tender_url' => $tender_url,
                            'notice_id' => null,
                ...
                if($private_tender && $tender_attachments)
                {
                    $filenames = [];
                    ...
                    foreach ($filenames as $filename) {
                        if($filename){
                            try{
                                ...
                                Log::error("Error processing attachment for private tender ID {$private_tender->private_tender_id}: " . $e->getMessage());
                            }
                        }
                    }
                }
