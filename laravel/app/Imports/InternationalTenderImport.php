...
class InternationalTenderImport implements ToCollection, WithValidation, WithStartRow
{
    ...
    {
        $this->existing_tender_nos = InternationalTender::pluck('tender_no')->toArray();
        ...
        foreach ($rows as $key => $row) {
            ...
            try {
                ...
                if (in_array($tender_no, $this->existing_tender_nos)) {
                    DuplicateInternationalTender::updateOrCreate([
                        'tender_no' => $tender_no,
                ...
                $tdr_fees = 0;
                try{
                    $international_tender = InternationalTender::updateOrCreate(
                        [   'tender_no' => $tender_no ],
                        [
                            'title' => $title,
                            ...
                            'state_id ' => null,
                            'tender_type_id' => null,
                            'international_notice_id' => null,
                            'category_id' => null,
                            'international_agency_id ' => null,
                            'tender_url' => $tender_url,
                            'notice_id' => null,
                ...
                if($international_tender && $tender_attachments)
                {
                    $filenames = [];
                    ...
                    foreach ($filenames as $filename) {
                        if($filename){
                            try{
                                ...
                                Log::error("Error processing attachment for international tender ID {$international_tender->international_tender_id}: " . $e->getMessage());
                            }
                        }
                    }
                }
